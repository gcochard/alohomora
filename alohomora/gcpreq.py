...
class DuoRequestsProvider(WebProvider):
    ...
    def kill_subprocess(self):
        LOG.debug('checking subprocess')
        if not self.subprocess.returncode:
            LOG.debug('subprocess still running, terminating...')
            self.subprocess.terminate()
            self.subprocess.wait()
            LOG.debug('subprocess exited with status {}'.format(self.subprocess.returncode))

    def login_one_factor(self, username, password):
        self.subprocess = subprocess.Popen(['gcloud', 'auth', 'login', '--no-launch-browser'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
        atexit.register(self.kill_subprocess)
        stderr = self.subprocess.stderr.peek().decode('UTF-8')
        LOG.debug(stderr)
        loginurl = stderr.split('\n')[2].strip()
        urlparts = loginurl.split('&')
        for i,str in enumerate(urlparts):
            if str.startswith('prompt'):
                urlparts[i] = 'login_hint={u}%40viasat.io'.format(u=username)
        self.url = '&'.join(urlparts)
        LOG.debug(self.url)
        self.session = requests.Session()

        (response, soup) = self._do_get(self.url)
        self.idp_url = response.url
        LOG.debug(self.idp_url)
        payload = {}
    ...
    def login_two_factor(self, response_1fa):
        ...
        LOG.debug('POSTing the SSO 2fa response')
        ...
        payload = {}
        action = self._get_form_action(soup)
        for inputtag in soup.find_all('input'):
            name = inputtag.get('name', '')
            value = inputtag.get('value', '')
            payload[name] = value

        LOG.debug('POSTing back to the SAML ACS endpoint with the following payload: {}'.format(payload))
        headers = {
            'Referer': response.url,
            'Content-Type': 'application/x-www-form-urlencoded'
        }
        # this redirects to the google SAML ACS url with the SAMLAssertion
        (response, soup) = self._do_post(action, data=payload, headers=headers)

        LOG.debug('history: {}'.format(response.history))
        LOG.debug('soup: {}'.format(soup))
        # and we have some weird 'continue' page that we have to get
        link = soup.find('a')
        if link is None:
            alohomora.die('Expected redirect link not found.')
        target = link.get('href', '')
        headers = {
            'Referer': response.url
        }
        LOG.debug('GETting the following URL: {}'.format(target))
        (response, soup) = self._do_get(target, headers=headers)

        LOG.debug('history: {}'.format([h.url for h in response.history]))
        LOG.debug('soup: {}'.format(soup))
        # this is yet another redirect page
        payload = {}
        action = self._get_form_action(soup)
        for inputtag in soup.find_all('input'):
            name = inputtag.get('name', '')
        ...
        LOG.debug('POSTing to the following URL: {} with the following payload: {}'.format(target, payload))
        (response, soup) = self._do_post(action, data=payload, headers=headers)

        LOG.debug('history: {}'.format([h.url for h in response.history]))
        LOG.debug('soup: {}'.format(soup))
        tag = soup.find('meta')
        target = tag.get('content').split("; ")[1].split('\'')[1][:-1]
        LOG.debug('redirecting to {}'.format(target))
        (response, soup) = self._do_get(target)
        LOG.debug('history: {}'.format([h.url for h in response.history]))
        raise Exception('foo')
        return (True, assertion)

        # IdP does a redirect to gcp, a POST with form field SAMLResponse filled out
        # We DO want to follow the redirect.  Need to capture the token that Google issues.
